...
public class OptionalDependenciesPluginIntegrationTests {
	...
	public void optionalConfigurationIsCreated() throws IOException {
		try (PrintWriter out = new PrintWriter(new FileWriter(this.buildFile))) {
			out.println(
					"plugins { id 'org.springframework.boot.optional-dependencies' }");
		...
		BuildResult buildResult = runGradle(this.temporaryFolder.getRoot(),
				"printConfigurations");
		assertThat(buildResult.getOutput()).contains("optional");
	}

	@Test
	public void optionalDependenciesAreAddedToMainSourceSetsCompileClasspath()
			throws IOException {
		optionalDependenciesAreAddedToSourceSetClasspath("main", "compileClasspath");
	}

	@Test
	public void optionalDependenciesAreAddedToMainSourceSetsRuntimeClasspath()
			throws IOException {
		optionalDependenciesAreAddedToSourceSetClasspath("main", "runtimeClasspath");
	}

	@Test
	public void optionalDependenciesAreAddedToTestSourceSetsCompileClasspath()
			throws IOException {
		optionalDependenciesAreAddedToSourceSetClasspath("test", "compileClasspath");
	}

	@Test
	public void optionalDependenciesAreAddedToTestSourceSetsRuntimeClasspath()
			throws IOException {
		optionalDependenciesAreAddedToSourceSetClasspath("test", "runtimeClasspath");
	}

	public void optionalDependenciesAreAddedToSourceSetClasspath(String sourceSet,
		...
		try (PrintWriter out = new PrintWriter(new FileWriter(this.buildFile))) {
			...
			out.println("    doLast {");
			out.println("        println sourceSets." + sourceSet + "." + classpath
					+ ".files");
			out.println("    }");
			out.println("}");
		}
